function AddOn()
{
    ...
    this.showUsage = function()
    {
        console.info([
            ...
            " - getPlaylist(int n)",
            "    Get n albums from filtered ones."
        ].join("\n"));
    }
    this.getPlaylist = function(n)
    {
        var playlist = [];
        var scores = [];
        var tr = $('tr:gt(0)');
        var trVisible = $('tr:gt(0):visible');
        trVisible.each(function(){
            for (var i = 0, l = $(this).index(); i < l; i++)
            {
                scores.push(l - 1); // Reduce 1 because tr:eq(0) is excluded.
            }
        });
        for (var i = 0; i < n; i++)
        {
        ...
        tr.hide();
        playlist.forEach(function(album){
            $(album).show();
        });
        stripeTable($('tr:gt(0):visible'));
        $('#wishlist > h2').text("playlist");
        $('#wishlist > .num').text(n);
    }
